...
static getHTML(pokemonObj){var pokemonID=pokemonObj.id.replace(/-\w+/,"");var pokemonTypes=pokemonObj.type.map(type=>`<div class="type ${type}" style="width:${pokemonObj.type.length>1?5:10}rem;">${FMT(type)}</div>`).join("");var moveList=pokemonObj.moves.map(move=>getMoveEntryHtml(move));var evolveStage=getEvolveStage(pokemonObj.evolution)!=""?`<div class="entry"><b style="background:#b7b7b7">進化階段</b>${getEvolveStage(pokemonObj.evolution)}</div>`:"";var evolveTime=getEvolveTime(pokemonObj.evolution)!=""?`<div class="entry"><b style="background:#b7b7b7">進化時間</b>${getEvolveTime(pokemonObj.evolution)}</div>`:"";var evolveMethod=getEvolveMethod(pokemonObj.evolution)!=""?`<div class="entry">${getEvolveMethod(pokemonObj.evolution)}</div>`:"";return `<div class="Pokemon">
            ...
            </div>`;function getEvolveStage(evolvoObj){if(!evolvoObj||!evolvoObj.stage)return "";switch(evolvoObj.stage){case "first":return "初階";case "second":return "二階";case "final":return "最終階";case "mega":return "超級進化";case "unknown":return "未知";}
return "-";}
...
return "-";}
function getEvolveMethod(evolvoObj){if(!evolvoObj)return "";var note=pokemonObj.evolution.note?` & <u>${pokemonObj.evolution.note}</u>`:"";if(evolvoObj.with)return `透過 <u>${evolvoObj.with}</u> 進化${note}`;if(evolvoObj.beauty)return `在 <u>美麗${evolvoObj.beauty}</u> 時進化${note}`;if(evolvoObj.loyalty)return `在 <u>忠誠度${evolvoObj.loyalty}</u> 時進化${note}`;if(evolvoObj.happiness)return `在 <u>幸福度${evolvoObj.happiness}</u> 時進化${note}`;if(evolvoObj.limitby)return `限定由 <u>${evolvoObj.limitby}</u> 進化${note}`;if(evolvoObj.by)return `經由 <u>${evolvoObj.by}</u> 進化${note}`;return '';}
