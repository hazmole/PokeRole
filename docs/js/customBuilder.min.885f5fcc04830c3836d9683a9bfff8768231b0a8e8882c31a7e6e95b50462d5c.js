...
return null;}}}
var i18n;function FMT(langkey){var val=i18n[langkey.toLowerCase()];return val?val:langkey;}
window.addEventListener("load",()=>{var isDomChanged=false;isDomChanged|=IconParser.parsePage();isDomChanged|=MoveParser.parsePage();if(isDomChanged)window.onscroll(true);var event=new CustomEvent("parsePage",{"detail":"finished parsing page."});window.dispatchEvent(event);});