class IconParser{static parsePage(){var article=document.getElementById("list-main");if(!article)return;var hasIcon=false;var raw_icon;while((raw_icon=article.innerHTML.match(/{@icon ([^\}]+)}/))!=null){var originText=raw_icon[0];var tagArr=raw_icon[1].split("|");var iconHtml=this.getIconHTML(tagArr);article.innerHTML=article.innerHTML.replace(originText,iconHtml);hasIcon=true;}
...
function FMT(langkey){var val=i18n[langkey.toLowerCase()];return val?val:langkey;}
window.addEventListener("load",()=>{IconParser.parsePage();});if(!!MoveList){window.addEventListener("load",()=>{MoveParser.parsePage();});}