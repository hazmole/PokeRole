...
static renderAbility(pokemonObj){var AbilityPanel=document.getElementById("AbilityList");var ability=null;if(AbilityPanel){if(AbilityList){var abilityName=pokemonObj.ability[pokemonObj.abilityIdx];ability=AbilityList.find(item=>item.name==abilityName&&item.tags[0]!="unknown|l");}
...
PokemonGenerator.renderAbility(pokemonObj);PokemonGenerator.renderMoves(pokemonObj);return true;function getHTML(pokemonObj){var pokemonID=pokemonObj.id.replace(/-\w+/,"");var pokemonTypes=pokemonObj.type.map(type=>`<div class="type ${type}" style="width:${pokemonObj.type.length>1?5:10}rem;">${FMT(type)}</div>`).join("");return `<div class="Pokemon">
