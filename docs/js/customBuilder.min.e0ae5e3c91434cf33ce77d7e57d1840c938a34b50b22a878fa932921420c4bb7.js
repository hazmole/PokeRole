...
return true;function getDomObjId(domObj,className){switch(className){case "Nature":case "Move":case "Ability":return domObj.getElementsByClassName("title").item(0).innerText;}
return 'unknown';}
...
window.addEventListener("load",()=>{MoveParser.parseTypeIcon();var isDomChanged=false;isDomChanged|=IconParser.parsePage();isDomChanged|=PokemonParser.parsePage();isDomChanged|=MoveParser.parsePage();isDomChanged|=AbilityParser.parsePage();if(isDomChanged)window.onscroll(true);var event=new CustomEvent("parsePage",{"detail":"finished parsing page."});window.dispatchEvent(event);});