...
window.addEventListener("load",()=>{MoveParser.parseTypeIcon();var isDomChanged=false;isDomChanged|=IconParser.parsePage();isDomChanged|=MoveParser.parsePage();isDomChanged|=PokemonParser.parsePage();if(isDomChanged)window.onscroll(true);var event=new CustomEvent("parsePage",{"detail":"finished parsing page."});window.dispatchEvent(event);});