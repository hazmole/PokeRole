...
static getHTML(pokemonObj){console.log(pokemonObj);var pokemonTypes=pokemonObj.type.map(type=>`<div class="type ${type}" style="width:${pokemonObj.type.length>1?5:10}rem;">${FMT(type)}</div>`).join("");var moveList=pokemonObj.moves.map(move=>getMoveEntryHtml(move));return `<div class="Pokemon">
            ...
            </div>`;function getEvolveStage(evolvoObj){if(!evolvoObj||!evolvoObj.stage)return "-";switch(evolvoObj.stage){case "first":return "初階";case "second":return "二階";case "final":return "最終階";case "mega":return "MEGA";}
return "-";}
...
function getAttr(attr){var text="";for(var i=0;i<attr.max;i++){text+=(i<attr.value)?"●":"○";if(i==4)text+=" ";}
return text;}}}
...
return true;function getDomObjId(domObj,className){switch(className){case "Nature":case "Move":return domObj.getElementsByClassName("title").item(0).innerText;}
return 'unknown';}
...
window.addEventListener("load",()=>{var isDomChanged=false;isDomChanged|=IconParser.parsePage();isDomChanged|=MoveParser.parsePage();isDomChanged|=PokemonParser.parsePage();if(isDomChanged)window.onscroll(true);var event=new CustomEvent("parsePage",{"detail":"finished parsing page."});window.dispatchEvent(event);});